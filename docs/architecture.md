...
La cadence beacon (~3 s) sert au join/roster ; la staleness (`STALE_MS` = 5 intervalles, 15 s) n’affecte que l’affichage roster et le pré-chauffage du standby overlay. **Le basculement appartient à la santé TCP** : la mort du `PeerLink` sur un lien LOCAL promeut l’overlay seulement si aucun sighting LAN frais n’existe (sinon on redial le LAN — les courses dual-dial ne doivent pas verrouiller l’overlay). Un **vrai** échec de dial LAN (timeout / refus), pas une annulation volontaire (`Socket closed`), bascule immédiatement sur Tailscale quand un endpoint overlay est connu.

Dial sortant : `TCP_NODELAY` partout, backoff exponentiel plafonné à 30 s (remis à zéro seulement si l’identité host/port change, une action utilisateur, ou un succès — pas à chaque heartbeat beacon). **Les deux pairs dialent** ; le premier Hello gagne. Le listen TCP utilise le port fixe **47101** (UDP discovery : **47100**) pour que le registre overlay survive un redémarrage d’app ; en cas d’échec de bind, repli sur un port éphémère. Un endpoint overlay sticky (host:port en mémoire) est invalidé après `ECONNREFUSED` ou plusieurs échecs consécutifs, et vidé au changement de réseau / `shutdown` — les seeds de probe UDP (IP roster) sont conservés. `promoteToOverlay` est idempotent : un tick de présence ne force pas un `forceRestart` sur le même endpoint déjà en cours de dial.

Changement de connectivité (`NetworkMonitor`) : oubli immédiat des sightings LAN **et** overlay live, vidage du registre TCP overlay, fermeture des sockets, reset du backoff, rebind beacon, re-dial — pas d’attente de stale 15 s. L’IP overlay annoncée préfère une iface `tun*` stable (`tun0` avant `tun1`) quand plusieurs adresses `100.x` coexistent.
