...
- **Audio** : codec Opus (`OpusCodec`, implémentation pure Java via Concentus pour compatibilité `minSdk`) par trames de 20 ms (`AudioCapture`/`AudioPlayback`), sur le flux **communication** VoIP (`USAGE_VOICE_COMMUNICATION`, `MODE_IN_COMMUNICATION`) avec annulation d'écho plateforme (`AcousticEchoCanceler` + NS + AGC), haut-parleur par défaut en mains libres et bascule automatique vers casque/Bluetooth s'ils sont branchés.
